...
public float GetSteering() => steering != null ? steering.ReadValue<float>() : 0f;

public float GetThrottle()
{
    if (gas == null) return 0f;
    float raw = gas.ReadValue<float>(); // 1 → -1
    return Mathf.Clamp01((1f - raw) / 2f);
}

public float GetBrake()
{
    if (brake == null) return 0f;
    float raw = brake.ReadValue<float>(); // -1 → 1
    return Mathf.Clamp01((raw + 1f) / 2f);
}
